...
#                                 shift = True
#                             else:
#                                 ss += hid2.get(byte, '')  # Use get to avoid KeyError
#                                 shift = False
#                         else:
#                             if byte == 2:  # Shift key
#                                 shift = True
#                             else:
#                                 ss += hid.get(byte, '')  # Use get to avoid KeyError
...
# def read_dimensions():
#     """Measure length and width using VL53L0X sensors."""
#     length = sensor_length.range / 100.0  # Convert from mm to cm
#     width = sensor_width.range / 100.0    # Convert from mm to cm
#     return length, width
...
#             if barcode:  # Only proceed if there is a barcode
...
#                 # Get dimensions
#                 length, width = read_dimensions()
#                 print(f"Length: {length:.2f} cm")
#                 print(f"Width: {width:.2f} cm")
#                 height = 12.8  # Replace with your height measurement logic if applicable
#                 print(f"Height: {height:.2f} cm")  # Assuming height is a constant value for now

#                 # Calculate CBM
#                 cbm = calculate_cbm(length, width, height)
#                 print(f"CBM: {cbm:.6f} m³")  # Print CBM with 6 decimal places for precision

#                 print("\n")  # Add a newline for better readability
#             time.sleep(1)  # Pause before the next scan
    ...
    try:
        while True:
            ...
            if barcode:  # Proceed only if barcode is read
                ...
                # Check if the weight is less than 1 kg
                if stable_weight < 1:
                    print("Please Put the Box first")
                    continue  # Go back to waiting for barcode if no box is placed
                
                # Wait for weight to stabilize for 2 seconds
                stable_weight = wait_for_stable_weight(stability_duration=2)
                print("Ready to scan")

                # Proceed with the rest of the process once stable
                print(f"Barcode: {barcode}")
                ...
                # Wait for weight to increase (new box)
                while True:
                    new_weight = get_stable_weight()
                    if new_weight > stable_weight + 0.05:  # Significant increase
                        print("Taring...")
                        hx.tare()  # Reset the scale
                        break
